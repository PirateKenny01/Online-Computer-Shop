...
<body>
    ...
    <form method="post" action="../controllers/adminCreateUserCheck.php" onsubmit="return validateSignup()">
        ...
        Role:
        <select name="role">
            <option value="customer" <?php if(isset($old['role']) && $old['role']=="customer"){echo "selected";} ?>>Customer</option>
            <option value="admin" <?php if(isset($old['role']) && $old['role']=="admin"){echo "selected";} ?>>Admin</option>
        </select>
        <p style="color:red;"><?php if(isset($errors['role'])){echo $errors['role'];} ?></p>
